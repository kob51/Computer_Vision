...
#        fig, ax = plt.subplots(1)
        penalty = 0
        g0 = np.linalg.inv(X@X.T + penalty*np.eye(X.shape[0])) @ X @ Y
        g0 = np.reshape(g0,(dy.size,dx.size))
        # corr0 = scipy.ndimage.correlate(img,g0)
        conv0 = scipy.ndimage.convolve(img,g0)
        # ax.imshow(corr0,cmap='gray')
#        ax.imshow(conv0,cmap='gray')
#        plt.show()
        
#        fig1, ax1 = plt.subplots(1)
        penalty = 1
        g1 = np.linalg.inv(X@X.T + penalty*np.eye(X.shape[0])) @ X @ Y
        g1 = np.reshape(g1,(dy.size,dx.size))
        # corr1 = scipy.ndimage.correlate(img,g1)
        conv1 = scipy.ndimage.convolve(img,g1)
        # ax1.imshow(corr1,cmap='gray')
